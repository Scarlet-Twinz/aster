    ...
    fn check_expression(&mut self, expression: &Expr) -> Type {
        match expression {
            ...
            Expr::Unary { operator, operand } => {
                ...
                match operator {
                    UnaryOp::Negate => {
                        self.require_type(operand_type, Type::Number, "unary '-' requires a number");
                        Type::Number
                    }
                    UnaryOp::Not => {
                        self.require_type(operand_type, Type::Boolean, "unary '!' requires a bool");
                        Type::Boolean
                    }
                }
            }
            ...
            Expr::Call { callee, arguments } => {
                ...
                match callee_name {
                    ...
                    None => {
                        self.error("only named functions can be called".into());
                        Type::Unknown
                    }
                }
            }
    ...
    fn check_binary(&mut self, operator: BinaryOp, left: Type, right: Type) -> Type {
        match operator {
            ...
            BinaryOp::And | BinaryOp::Or => {
                self.require_type(left, Type::Boolean, "logical operators require bools");
                self.require_type(right, Type::Boolean, "logical operators require bools");
                Type::Boolean
            }
        }
    }

    fn require_type(&mut self, actual: Type, expected: Type, message: &str) {
        if actual != Type::Unknown && actual != expected {
            self.error(format!("{} (found {}, expected {})", message, actual, expected));
        }
    }
