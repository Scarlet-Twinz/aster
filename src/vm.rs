...
impl Vm {
    ...
    pub fn run(mut self, chunk: &Chunk) -> Result<Vec<String>, VmError> {
        while self.ip < chunk.code.len() || !self.frames.is_empty() {
            ...
            match instruction {
                ...
                OpCode::LoadLocal(index) => { let value=match self.frames.last(){Some(frame)=>frame.locals.get(index).cloned(),None=>None}.ok_or_else(||self.error("invalid local variable index"))?; self.stack.push(value); }
                OpCode::StoreLocal(index) => { let value=self.pop()?; match self.frames.last_mut(){Some(frame) if index<frame.locals.len()=>frame.locals[index]=value,Some(_)=>return Err(self.error("invalid local variable index")),None=>return Err(self.error("local variable outside function"))} }
                OpCode::Pop => { self.pop()?; }
    ...
    fn fetch_instruction(&mut self,chunk:&Chunk)->Result<OpCode,VmError>{
        if self.frames.is_empty(){ if self.ip>=chunk.code.len(){return Err(self.error("instruction pointer escaped program"));} let instruction=chunk.code[self.ip];self.ip+=1;Ok(instruction) }
        else { let frame=self.frames.last_mut().expect("frame exists"); let function_index=frame.function_index; let code_len=match chunk.functions.get(function_index){Some(f)=>f.code.len(),None=>return Err(self.error("invalid function index"))}; if frame.ip>=code_len{return Err(self.error("instruction pointer escaped function"));} let instruction=chunk.functions[function_index].code[frame.ip];frame.ip+=1;Ok(instruction) }
    }
    ...
    fn call(&mut self,chunk:&Chunk,function_index:usize)->Result<(),VmError>{let arity=match chunk.functions.get(function_index){Some(f)=>f.arity,None=>return Err(self.error("invalid function index"))};if self.stack.len()<arity{return Err(self.error("not enough arguments for function call"));}let mut args=Vec::with_capacity(arity);for _ in 0..arity{args.push(self.pop()?);}args.reverse();self.frames.push(Frame{function_index,ip:0,locals:args});Ok(())}
